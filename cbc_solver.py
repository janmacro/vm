...
def optimize_lineup_ilp(
    swimmers: List[str],
    points: Dict[Tuple[str, Event], float],   # points[(swimmer, event_enum)]
...
) -> Tuple[float, float, List[Tuple[int,int,Event,str,float]]]:
    """
    Returns:
        best_points, best_congestion, assignment as list of
        (slot_index, seg_idx, event_enum, swimmer, points_for_that_slot) sorted by (seg).
    """
    ...
    # --------- PASS 2: fix points, minimize congestion ---------
    ...
    for seg_idx, seg in enumerate(segments):
        ...
        for L in congestion_window_sizes:
            if L <= 1 or L > seg_len:
                continue
            w = congestion_weights.get(L, 1)
            for start in range(seg_len - L + 1):
                window_slots = seg_slots[start:start+L]
                for s in swimmers:
                    count = solver2.IntVar(0, L, f"cnt_s{swimmers.index(s)}_g{seg_idx}_st{start}_L{L}")
                    solver2.Add(count == solver2.Sum(x2[(s, sl)] for sl in window_slots))
                    excess = solver2.IntVar(0, L - 1, f"exc_s{swimmers.index(s)}_g{seg_idx}_st{start}_L{L}")
    ...
    best_penalty = solver2.Objective().Value()
    ...
    return best_points, best_penalty, assignment
...
if __name__ == "__main__":
    ...
    # # Solve
    # best_pts, best_pen, lineup = optimize_lineup_ilp(
    #     swimmers=swimmers,
    ...
    # # Print
    # print(f"Best total points: {best_pts:.0f}")
    # print(f"Min congestion penalty (tie-break): {best_pen:.0f}\n")
    # current_seg = -1
    # check_points = 0
    ...
    #     check_points += pts
    # print(f"\nCheck sum of points: {check_points:.0f}")
